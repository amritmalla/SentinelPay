...
class OutboxRelayIT {
    ...
    @Test
    void relay_publishesProviderHealthChangedWithProviderKey() {
        UUID aggregateId = UUID.randomUUID();
        ProviderOutboxEntity row = new ProviderOutboxEntity();
        row.setAggregate("provider_health");
        row.setAggregateId(aggregateId);
        row.setEventType("provider.health.changed");
        row.setPayload(
                """
                {
                  "provider": "stripe",
        ...
        try (KafkaConsumer<String, EventEnvelope> consumer = createConsumer()) {
            consumer.subscribe(List.of("provider.health.changed"));
            ...
            assertThat(record.key()).isEqualTo("stripe");
            assertThat(record.value().source()).isEqualTo("provider-service");
        }
    }
    ...
    private KafkaConsumer<String, EventEnvelope> createConsumer() {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "provider-outbox-it-" + UUID.randomUUID());
